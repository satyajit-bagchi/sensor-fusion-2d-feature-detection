...
int main(int argc, const char* argv[]) {
    ...
    for (auto& detectorType : detectorTypes) {
        ...
        for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex;
            ...
            numDetectedKeypoints.push_back(keypoints.size());
            //// EOF STUDENT ASSIGNMENT
            ...
            if (bLimitKpts) {
                ...
                if (detectorType.compare("SHITOMASI") ==
                    0) {  // there is no response info, so keep the first 50 as
                          // they
                          // are sorted in descending quality order
                    keypoints.erase(keypoints.begin() + maxKeypoints,
                                    keypoints.end());
                }
            ...
            // push keypoints and descriptor for current frame to end of data
            // buffer
            ...
            {
                // std::cout << "BUFFER READY, CALCULATING MATCHES" <<
                // std::endl;
                ...
                /// distance ratio filtering with t=0.8 in file matching2D.cpp
                ...
                if (bVis) {
                    cv::Mat matchImg =
                        ((dataBuffer.end() - 1)->cameraImg).clone();
